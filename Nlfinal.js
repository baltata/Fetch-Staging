...
function startCountdownInIframe(iframeDocument, elementId, nextAttemptTime, submitButton, blurElement, outcomeElement) {
    function updateCountdown() {
        ...
        if (timeRemaining > 0) {
            ...
            if (submitButton) {
                submitButton.value = `Attends ${timeString}`;
            }

            // Update the blur countdown text if needed
            if (blurElement) {
                const blurCountdownElement = iframeDocument.getElementById(`${elementId}_blur`);
                if (blurCountdownElement) {
                    blurCountdownElement.innerHTML = timeString;
                }
            }
        } else {
            ...
            if (submitButton) {
                ...
                submitButton.classList.remove('disabled-submit-button'); // Remove pulsating effect
...
function applyBlurAndGreyishToAllIframes(nextAttemptTime) {
    ...
    for (let i = 0; i < contenairexercice.length; ++i) {
        ...
        if (submitButton && !submitButton.disabled) {
            ...
            submitButton.style.color = "white";  // Ensure text is visible
            submitButton.style.transition = "all 0.5s ease";  // Smooth transition
        ...
        if (correction) {
                ...
                correction.insertAdjacentHTML('beforeend', `
                    <div class="blur">
                        <div class="gosabonner">
                            Tu as atteint le maximum d'erreurs 😥 Attends <b><span id="${identifiantExercice}_blur"></span></b>.
                            <br> ou 
            ...
            // Run the countdown inside the iframe context
            setTimeout(function () {
                startCountdownInIframe(iframeDocument, identifiantExercice, nextAttemptTime, submitButton, blurElement, correction);
            }, 500);
        } else {
            // Run the countdown inside the iframe context for the submit button only
            if (submitButton && nextAttemptTime) {
                setTimeout(function () {
                    startCountdownInIframe(iframeDocument, identifiantExercice, nextAttemptTime, submitButton, null, null);
                }, 500);
            }
        }
    }
        ...
        if (checkStatus(identifiantExercice)) {
            ...
            if (submitButton && maxAttemptsReached) {
                ...
                if (!submitButton.disabled) {
                    ...
                    submitButton.style.color = "white";  // Ensure text is visible
                    submitButton.style.transition = "all 0.5s ease";  // Smooth transition
                    submitButton.classList.add('disabled-submit-button'); // Add pulsating effect
                }
            }
            ...
            } else if (maxAttemptsReached && nextAttemptTime) {
                // Run the countdown inside the iframe context for the submit button only
                setTimeout(function () {
                    startCountdownInIframe(iframeDocument, identifiantExercice, nextAttemptTime, submitButton, null, null);
                }, 500);
